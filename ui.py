...
class JarvisUI:
    def __init__(self, face_path, size=None):
        ...
        # Rosto holográfico animado (config/settings.json -> hologram_face)
        self._holo         = SETTINGS.get("hologram_face", True)
        self._blink_f      = 1.0        # 1 = olho aberto, 0 = fechado
        self._blink_total  = 10
        self._blink_frames = 0
        self._next_blink   = random.randint(90, 320)
        self._gaze_x       = 0.0        # direção do olhar (-1..1)
        self._gaze_y       = 0.0
        self._next_gaze    = 0
        self._mouth_open   = 0.0        # abertura da boca (0..1)
    ...
    def _animate(self):
        ...
        # ── rosto holográfico: piscar, olhar e boca ──────────────
        if self._holo:
            # piscar: triângulo fecha-abre em ~10 frames
            if self._blink_frames > 0:
                self._blink_frames -= 1
                p = 1 - self._blink_frames / self._blink_total
                self._blink_f = abs(math.cos(math.pi * p))
            else:
                self._blink_f += (1.0 - self._blink_f) * 0.5
                if self.tick >= self._next_blink:
                    self._blink_total  = 10
                    self._blink_frames = self._blink_total
                    self._next_blink   = self.tick + random.randint(90, 320)

            # olhar: idle vagueia, THINKING olha pra cima, SPEAKING encara
            if self.tick >= self._next_gaze:
                if self.muted:
                    tx, ty = random.uniform(-0.3, 0.3), random.uniform(-0.1, 0.1)
                elif self._jarvis_state == "THINKING":
                    tx, ty = random.uniform(-0.6, -0.2), random.uniform(-0.8, -0.4)
                elif self.speaking:
                    tx, ty = random.uniform(-0.15, 0.15), random.uniform(-0.1, 0.1)
                else:
                    tx, ty = random.uniform(-0.7, 0.7), random.uniform(-0.4, 0.4)
                self._gaze_x, self._gaze_y = tx, ty
                self._next_gaze = self.tick + random.randint(35, 140)

            # boca: só mexe quando o JARVIS está falando
            if self.speaking:
                if self.tick >= self._next_mouth:
                    self._mouth_target = random.uniform(0.15, 1.0)
                    self._next_mouth   = self.tick + random.randint(3, 7)
                self._mouth_open += (self._mouth_target - self._mouth_open) * 0.45
            else:
                self._mouth_open *= 0.72
    ...
    def _draw(self):
        ...
        if self._holo:
            self._draw_holo_face(c, FCX, FCY, FW)
        elif self._has_face:
    ...
    def _draw_holo_face(self, c, FCX, FCY, FW):
        """Rosto holográfico: olhos que piscam e seguem o olhar,
        boca que mexe sincronizada com a fala."""
        muted = self.muted
        base  = (255, 40, 90) if muted else (0, 212, 255)

        def bc(a):
            a = max(0, min(255, int(a)))
            return self._ac(base[0], base[1], base[2], a)

        S  = FW * 0.40 * self.scale   # meia-largura da cabeça
        HW = S
        HH = S * 1.16

        # halo de fundo (radial)
        for i in range(6, 0, -1):
            r2 = int(HW * 1.5 * i / 6)
            c.create_oval(FCX-r2, FCY-r2, FCX+r2, FCY+r2,
                          fill=bc(self.halo_a * 0.045 * (7 - i)), outline="")

        # contorno da cabeça (wireframe duplo)
        c.create_oval(FCX-HW, FCY-HH, FCX+HW, FCY+HH,
                      outline=bc(self.halo_a * 1.6), width=2)
        m = HW * 0.08
        c.create_oval(FCX-HW+m, FCY-HH+m, FCX+HW-m, FCY+HH-m,
                      outline=bc(self.halo_a * 0.55), width=1)
        # linha de maxilar
        c.create_arc(FCX-HW*0.9, FCY-HH*0.55, FCX+HW*0.9, FCY+HH*0.85,
                     start=210, extent=120, style="arc",
                     outline=bc(self.halo_a * 0.45), width=1)

        # ── OLHOS ──────────────────────────────────────────────────
        ex_off = HW * 0.40
        ey     = FCY - HH * 0.22
        e_rx   = HW * 0.155
        e_ry   = HW * 0.19 * self._blink_f
        bf     = max(0.05, self._blink_f)

        for sx in (-1.0, 1.0):
            ex = FCX + sx * ex_off
            # glow ao redor do olho
            for gi in (3, 2):
                gr = e_rx * gi / 2.2
                c.create_oval(ex-gr, ey-gr*0.9, ex+gr, ey+gr*0.9,
                              outline=bc(self.halo_a * (0.22 if gi == 3 else 0.45)),
                              width=1)
            # íris deslocada pelo olhar
            gx = ex + self._gaze_x * e_rx * 0.55
            gy = ey + self._gaze_y * e_ry * 0.9
            iris_r = e_rx * 0.62
            c.create_oval(gx-iris_r, gy-iris_r*0.8*bf,
                          gx+iris_r, gy+iris_r*0.8*bf,
                          fill=bc(self.halo_a * 0.85), outline="")
            # pupila (núcleo brilhante)
            pr = e_rx * 0.30
            c.create_oval(gx-pr, gy-pr*bf, gx+pr, gy+pr*bf,
                          fill=self._ac(200, 250, 255,
                                        min(255, int(self.halo_a * 2))),
                          outline="")
            # pálpebra fechando: vira uma linha
            if bf < 0.55:
                c.create_line(ex-e_rx, ey, ex+e_rx, ey,
                              fill=bc(self.halo_a), width=2)
        ...
        if op > 0.04:
            oh = 2 + op * HH * 0.16
            c.create_oval(FCX-mw, my-oh, FCX+mw, my+oh,
                          outline=bc(170 + op * 60), width=2,
                          fill=bc(30 * op))
            c.create_oval(FCX-mw*0.55, my-oh*0.45, FCX+mw*0.55, my+oh*0.45,
                          fill=bc(80 * op), outline="")
        else:
            c.create_line(FCX-mw, my, FCX+mw, my,
                          fill=bc(self.halo_a), width=2)
        # queixo
        c.create_arc(FCX-HW*0.5, my+HH*0.05, FCX+HW*0.5, my+HH*0.35,
                     start=200, extent=140, style="arc",
                     outline=bc(self.halo_a * 0.4), width=1)

        # ── scanlines do holograma ─────────────────────────────────
        scan_y = (self.tick * 1.7) % (HH * 2)
        for i in range(3):
            yy = FCY - HH + ((scan_y + i * HH * 0.66) % (HH * 2))
            c.create_line(FCX-HW, yy, FCX+HW, yy,
                          fill=bc(24), width=1)
